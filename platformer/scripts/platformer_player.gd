...
func _physics_process(delta: float) -> void:
	...
	if Game.change == true:
		if Input.is_action_just_pressed("change") and cam_change == true:
			cam.position.y += 1000
			cam_change = false
			Game.change_signal()
			$change_voice.play()
		elif Input.is_action_just_pressed("change") and cam_change == false:
			cam.position.y -= 1000
			cam_change = true
			Game.change_signal()
			$change_voice.play()
...
func _collect_item(item: Node2D) -> void:
	...
	current_item = item
	for area in get_tree().get_nodes_in_group("place_area"):
		area._update_hint()
